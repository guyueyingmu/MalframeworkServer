...
<!--    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"> -->
    ...
    <title>Control, <?php echo $_SESSION['UserName']; ?> </title>
    ...
    <script src="http://api.map.baidu.com/api?v=2.0&ak=rwDOdHrQXL0yfLqvdbXPXuZK"></script>
<!--    <script>
var map;
function initialize() {
  var mapOptions = {
...
//	}
	...
	<?php if($_SESSION['AutoScrollTextBox']){ echo "scroll='auto'"; } else { echo "scroll='manual'"; } ?>
	...
	function autorefresh(uid) {
	  refresh = setInterval(function() { getHistory(uid); autoScroll(); }, <?php echo $_SESSION['MessageBoxRefreshSpeed']; ?>);
	}
...
$(function(){
	...
	<script type='text/javascript'>
     $(document).ready(function () {
	     ...
	     autorefresh("");
         mBaiduLocate();
		 setInterval(refreshTable, <?php echo $_SESSION['DevicesTableRefreshSpeed']; ?>);
		 setInterval(refreshFileTable, <?php echo $_SESSION['FilesTableRefreshSpeed']; ?>);
         setInterval(mBaiduLocate, 5000);
     });
    </script>
  </head>
  ...
  <div class="modal fade" id="displayImages">
    ...
    <script type="text/javascript">	
        bMap = new BMap.Map("map-canvas");
        bMap.centerAndZoom(new BMap.Point(116.331398, 39.897445), 11);
        bMap.enableScrollWheelZoom(true);
        bMap.enableDoubleClickZoom();

        function mBaiduLocate(){
            bMap.clearOverlays();
          //  var new_point = new BMap.Point(106, 39);
          //  var marker = new BMap.Marker(new_point);
          //  bMap.addOverlay(marker);
          //  bMap.panTo(new_point);
          //  var infoWindow = new BMap.InfoWindow("123123",{enableMessage:false});
          //  marker.addEventListener("click", function(){this.openInfoWindow(infoWindow);});
            if(markersArray){
                for(i in markersArray){
                    bMap.addOverlay(markersArray[i]);
                }
            }
        }

        function bmapsmarker(lat, logi, title){
            var new_point = new BMap.Point(logi, lat);
            var marker = new BMap.Marker(new_point);
            //var infoWindow = new BMap.InfoWindow(title ,{enableMessage:false});
            //marker.addEventListener("click", autoZoom(marker));
        ...
        function select(bot, lat, longi) { 
            if (document.getElementById(bot).innerHTML == '-') { 
                ...
                //Add 4 lines for BaiduMap
                for(var i in markersArray){
                    if(markersArray[i].getTitle() == bot){
