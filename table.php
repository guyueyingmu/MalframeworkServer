...
<html>
  ...
  <?php foreach ($connect->query($getslaves) as $row) {
    ...
    $offlineminutes = $_SESSION['OffLineMinutes'];
    $timezonesetting = $_SESSION['TimeZoneSetting'];
    
    $today = time();
    //$today = strtotime(date("Y-m-d H:i:s"));
	$origdate = strtotime($row['update']);
	$secdiff = $today - $origdate;
	$mindiff = $secdiff / 60 - 8 * 60 * 2;

    echo 'todat:' . $today . '|' . 'oridate:' . $origdate . '|' . 'secdiff:' . $secdiff . '|' . 'mindiff:' . $mindiff . '|' . 'offlineminuter:' . $offlineminutes . '-----' . $timezonesetting.'</br>';
